...
Importing this package registers every family; `overrides` imports it at the
bottom, so importing that module alone yields a populated registry.
Module names mirror `models/`.

The import order is the **registration order** -- the definition order these
families had in `overrides.py`. An architecture may match several providers
and the later one wins, so alphabetising this list would change resolution.
"""
